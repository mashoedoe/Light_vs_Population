...
town_shp <- readOGR(dsn = "/media/mashudu/A296E14C96E12191/Copy/git/Light_vs_Population/SHP/LocalMunicipalities2011.shp", 
...
server_shp <- function(input, output, session) {
...
#          addPolygons(data = town_shp,
#                      popup = paste(town_shp@data$MUNICNAME,
#                                    "<br>",
#                                    "Size:",
#                                    town_shp@data$AREA,
#                                    " km<sup>2</sup>")
...
#       addPopups(data = t(sapply(slot(town_shp, 'polygons'), function(i) slot(i, 'labpt'))) , popup = paste(town_shp@data$MUNICNAME,
#                               "<br>",
#                               "Size:",
#                               town_shp@data$AREA,
#                               " km<sup>2</sup>"))
  ...
  observeEvent(input$map1_geojson_mouseover, {
    v$msg1 <- paste("Mouse is over", input$map1_geojson_mouseover$featureId)
      })
  observeEvent(input$map1_geojson_mouseout, {
    v$msg1 <- ""
  })
   observeEvent(input$map1_geojson_click, {
     v$msg2 <- paste("Clicked on", input$map1_geojson_click$featureId)
        })
...
#     }
  ...
  observeEvent(input$map1_geojson_mouseover, {
    v$msg5 <- paste("Mouse coordinates: Lat", round(input$map1_popup_mouseover$lat, digits = 3), 
                    "Long", round(input$map1_polygon_mouseover$lng, digits = 3))
  })
  observeEvent(input$map1_geojson_mouseout, {
    v$msg5 <- ""
  })
